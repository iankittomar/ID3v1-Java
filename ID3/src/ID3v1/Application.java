...
public class Application {
   static  final String encoding = "Cp437";
	public static void main(String args[])throws IOException {
	    ...
	    System.out.println("Enter the MP3 file Directory");
	    String path = sc.nextLine();
		FileExtract(path);
	}
    
    public static void FileExtract(String path)throws IOException {                                                                                                       //Files Extreacting
        File dir = new File(path);
		File [] list = dir.listFiles();
		for(File file : list) {                                                 // CHECKING FOR FILE OR DIR
			if(file.isDirectory()) {
			    ...
			    if (ext.equals("mp3")) {
                    ID3(file);  // Function all for ID3v1
		        }
			}
        }
    }
    /* 
    * Prints the Tag Details if Tag is present
    */
    public static void ID3(File file) throws IOException { 
        if(ID3exists(file)) {
            ...
            //System.out.println(Genre.getGenreByByteId((byte) tag.charAt(124)) + "  " + tag.substring(30, 60).trim()+ "  " + tag.substring(0, 30).trim());
			System.out.println("Name of Genre- " + Genre.getGenreByByteId((byte) tag.charAt(124))+ "  Interpreter- "+tag.substring(30, 60).trim()+ "  Title- "+tag.substring(0, 30).trim());
        }
    }

    public static boolean ID3exists(File file)throws IOException {
